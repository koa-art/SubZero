<p align="center">
  <img src="screenshots/icon.png" width="120" alt="SubGuard Logo"/>
</p>

<h1 align="center">🛡 SubGuard</h1>
<p align="center">
...
市面上大多数订阅管理APP要么收费、要么充满广告、要么要求注册账号收集你的数据。**SubGuard 完全开源、完全本地、完全免费。**

- Netflix、Spotify、iCloud、健身房... 每月自动扣费，到底花了多少？
- 什么时候到期续费？会不会忘了取消被白白扣钱？
- 哪些订阅其实用不上，该砍掉了？

SubGuard 帮你回答这些问题——**所有数据只存在你的手机上**。
...
```
SubGuard/
├── app/src/main/java/com/subguard/app/
...
```bash
# Clone
git clone https://github.com/koa-art/SubGuard.git
cd SubGuard
...
SubGuard, like our sister project [DailyCheckIn](https://github.com/koa-art/DailyCheckIn), uses SharedPreferences + JSON instead of SQLite. For a personal subscription tracker with at most a few hundred records, JSON storage is simpler, faster to develop, and completely sufficient.
